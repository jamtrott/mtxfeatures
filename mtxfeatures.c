 ...
 *
 * Compute various features of sparse matrices, such as bandwidth,
 * profile, and so on.
 *
 * Authors:
    ...
    FILE * f)
{
    ...
    fprintf(f, "\n");
    fprintf(f, " Compute sparse matrix features.\n");
    fprintf(f, "\n");
...
int main(int argc, char *argv[])
{
    ...
    /* 5. compute matrix features */
    if (args.verbose > 0) {
        fprintf(stderr, "computing matrix features:\n");
        clock_gettime(CLOCK_MONOTONIC, &t0);
    }
...
#ifdef _OPENMP
    #pragma omp parallel for reduction(+:rowsizesd)
#endif
    for (idx_t i = 0; i < num_rows; i++) {
    ...
    if (args.bandwidth) {
        idx_t bandwidth = 0;
        err = csrbandwidth(&bandwidth, num_rows, num_columns, csrsize, rowsizemin, rowsizemax, csrrowptr, csrcolidx, csra, csrad);
        if (err) {
            if (args.verbose > 0) fprintf(stderr, "\n");
            fprintf(stderr, "%s: %s\n", program_invocation_short_name, strerror(err));
            free(csrad); free(csra); free(csrcolidx); free(csrrowptr);
        ...
        fprintf(stdout, "bandwidth: %'"PRIdx"\n", bandwidth);
    }

    if (args.profile) {
        int64_t profile = 0;
        err = csrprofile(&profile, num_rows, num_columns, csrsize, rowsizemin, rowsizemax, csrrowptr, csrcolidx, csra, csrad);
        if (err) {
            if (args.verbose > 0) fprintf(stderr, "\n");
            fprintf(stderr, "%s: %s\n", program_invocation_short_name, strerror(err));
            free(csrad); free(csra); free(csrcolidx); free(csrrowptr);
    ...
    if (args.verbose > 0) {
        clock_gettime(CLOCK_MONOTONIC, &t1);
        fprintf(stderr, "done computing features in %'.6f seconds\n", timespec_duration(t0, t1));
    }
